...
JSON_LLM_TOOL_TIMEOUT_SECONDS = 60.0
FINAL_SYNTHESIS_LLM_TOOL_TIMEOUT_SECONDS = 180.0
MAX_LITE_SEARCH_ROUNDS = 3
TASK_TOTAL_BUDGET_SECONDS                 = 270.0
# The validator kills the run outright at 300s (timeout_miner_owned) — an LLM
# call still in flight then scores 0.0 with no fallback. Every llm_chat timeout
# is therefore clamped to the time left before the task deadline; below the
# floor the call is skipped so the run can still finish with a deterministic
# answer from already-accepted evidence.
MIN_JSON_LLM_CALL_TIMEOUT_SECONDS         = 8.0
MIN_FINAL_SYNTHESIS_TIMEOUT_SECONDS       = 12.0
...
async def _synthesize_final_answer(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    synthesis_timeout = _clamped_llm_timeout(
        FINAL_SYNTHESIS_LLM_TOOL_TIMEOUT_SECONDS, state=state,
        floor_seconds=MIN_FINAL_SYNTHESIS_TIMEOUT_SECONDS,
        reserve_seconds=FINAL_SYNTHESIS_RESERVE_SECONDS,
    )
    if synthesis_timeout is None:
    ...
    try:
        response = await llm_chat(
            ...
            thinking=FINAL_SYNTHESIS_THINKING, timeout=synthesis_timeout,
        )
...
def _clamped_llm_timeout(
    base_timeout: float, *, state: ResearchRunState, floor_seconds: float,
    reserve_seconds: float = 0.0,
) -> float | None:
    """Timeout for an llm_chat call that cannot outlive the task deadline.

    Returns None when the remaining budget is below ``floor_seconds`` — the
    caller must skip the call instead of starting one it cannot finish.
    """
    deadline = getattr(state, "deadline", None)
    if deadline is None:
        return base_timeout
    remaining = deadline - perf_counter() - reserve_seconds
    if remaining < floor_seconds:
        return None
...
async def _call_json_llm_with_retry(
    *, messages: list[dict[str, str]], model: str, temperature: float,
    ...
    for attempt_index in range(max_attempts):
        call_timeout = _clamped_llm_timeout(
            JSON_LLM_TOOL_TIMEOUT_SECONDS, state=state,
            floor_seconds=MIN_JSON_LLM_CALL_TIMEOUT_SECONDS,
        )
...
# slot: harnyx 2026-07-12T15:05:08+00:00

# portfolio: openrouter/parallel
_PORTFOLIO_TAG = "e6e5d4d970d30404"
